...
class CauchyCombinationConfig(GWASSumstatsConfig, ConfigWithAutoPaths):
    ...
    def _init_annotation_list(self):
        ...
        if not self.annotation and not self.cauchy_annotations:
            raise ValueError(
                "At least one of 'annotation' or 'cauchy_annotations' must be provided."
            )
        ...
        for trait in self.trait_name_list:
            ...
            if ldsc_input_file.exists():
                ...
                raise FileNotFoundError(
                    f"LDSC result file not found for {trait}: {ldsc_input_file}"
                )

        if not traits_dict:
            raise FileNotFoundError(
                f"No valid LDSC result files found for the specified traits in {self.ldsc_save_dir}"
            )
...
def check_cauchy_done(config: CauchyCombinationConfig, trait_name: str) -> bool:
    ...
    for annotation in config.annotation_list:
        anno_result = config.get_cauchy_result_file(
            trait_name, annotation=annotation, all_samples=True
        )
        sample_result = config.get_cauchy_result_file(
            trait_name, annotation=annotation, all_samples=False
        )
