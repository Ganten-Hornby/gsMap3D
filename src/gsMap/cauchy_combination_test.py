...
from gsMap3D.config import CauchyCombinationConfig
...
def acat_test(pvalues, weights=None):
    ...
    if any([i < 1e-15 for i in pvalues]) == False:
        ...
        cct_stat += sum(weights[is_large] *
                        np.tan((0.5 - pvalues[is_large]) * np.pi))
...
def run_Cauchy_combination(config: CauchyCombinationConfig):
    ...
    if config.meta is None:
        if config.annotation is None:
            ValueError("Please provide the cell annotation for the Cauchy test.")
            
        # Load the spatial data
        logger.info(
            f"------Loading ST data of {config.hdf5_with_latent_path}...")
        adata = sc.read_h5ad(f"{config.hdf5_with_latent_path}")
        common_cell = np.intersect1d(ldsc.index, adata.obs_names)
        adata = adata[common_cell]
    ...
    for ct in np.unique(ldsc.annotation):
        ...
        if 0 < n_remove < len(p_temp) * 0.05:
            logger.info(
                f"Remove {n_remove}/{len(p_temp)} outliers (median + 3IQR) for {ct}."
            )
    ...
    p_tissue.columns = ["p_cauchy", "p_median", 'n_cell',"annotation"]
    ...
    output_file = output_dir / \
        f"{config.sample_name}_{config.trait_name}.Cauchy.csv.gz"
    p_tissue.to_csv(
        output_file,
        compression="gzip",
        index=False,
