...
class ModelTrain:
    def __init__(self, model, optimizer, distribution, mode, lr, model_path):
        self.model = model
    ...
    def train(self, n_epochs, patience):
        ...
        for epoch in pbar:
            ...
            if loss_track.early_stop:
                print(
                    f"Stop training, as {self.mode} validation loss has not decreased for {patience} consecutive steps."
                )
                break
